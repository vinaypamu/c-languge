...
int main()
{
	...
	GPIOC_PUPDR |=0x00050000;//PUPDR_UP  anf PUPDR_DR intial value low seting to high thorugh pull up
	GPIOB_ODR |= (0x3<<13); //RED_LED  and GREEN_LED set to off
	while(1)
	{
			if(!(GPIOC_IDR & 0x01<<8))// IDR IS 1 to update 0
			{
				GPIOB_ODR &= ~(0x1<<13);	//RED_LED set to on	
				delay(1000);
			}
			if(!(GPIOC_IDR & 0x01<<9)) // IDR IS 1 to update 0 
			{
